...
from jupyter_ai_magics.hc import hardcoded_outputs, CodeCell, MarkdownCell
...
class BiomeMagics(AiMagics):
    ...
    def display_output(self, output, display_type, md):
        ...
        if BiomeMagics.HARDCODE:
            ...
            cell_list = hardcoded_outputs[BiomeMagics.output_counter]
            ...
            for cell in cell_list:
                if isinstance(cell, CodeCell):
                    ...
                    ip = get_ipython()
                    ip.payload_manager.write_payload(new_cell_payload)
                    return None #TODO: Consider multiple cells.
                else:
                    DisplayClass = DISPLAYS_BY_FORMAT.get(cell.cell_type, None)
                    if DisplayClass is not None:
                        return DisplayClass(cell.content, metadata=md)
        else:
            # If not hardcoded, fall back to the original or alternative logic
            return super().display_output(output, display_type, md)
